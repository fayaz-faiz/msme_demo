...
export function AddToCartButton({
  ...
  >({});
  const [lastCustomizationPayload, setLastCustomizationPayload] = useState<
  ...
  const getApiErrorMessage = (error: unknown, fallback: string) => {
    if (typeof error === "object" && error !== null) {
      ...
      return (
        maybeError.response?.data?.message || maybeError.message || fallback
      );
    }
    return fallback;
  };
  ...
  const updateServerCart = async (
    nextCount: number,
    customizationPayload: Record<
      string,
      string | string[]
    > = lastCustomizationPayload,
    ...
    try {
      ...
      const isCartEmptyResponse =
        statusCode === 406 || /cart\s*is\s*empty/i.test(message);

      if (ok) {
        const cartId = String(result?.data?.data?.cart_id || "");
        if (cartId) {
          try {
            const cartResp = (await postCartByIdWeb({
              cart_id: cartId,
            })) as CartByIdResponse;
            const totalAmount = Number(cartResp?.data?.data?.total_amount ?? 0);
            const cartItemCount = Number(
              cartResp?.data?.data?.item_count ?? itemCount,
            );
            dispatch(
              setCartSummary({ cartId, totalAmount, itemCount: cartItemCount }),
            );
          } catch {
      ...
      const message = String(
        getApiErrorMessage(error, "Unable to update cart."),
      );
      const isCartEmptyResponse =
        statusCode === 406 || /cart\s*is\s*empty/i.test(message);
  ...
  const buildInitialSelections = (groups: CustomizationGroup[]) => {
    ...
    groups.forEach((group) => {
      ...
      const fallbackDefault =
        defaults.length > 0
          ? defaults
          : group.type === "SINGLE" && group.min
            ? ([options[0]?.id].filter(Boolean) as string[])
            : [];
  ...
  const fetchCustomizations = async () => {
    ...
    return (result?.data?.data?.customizationItems || []).filter(
      (group) => group?.groupId,
    );
  };
  ...
  const updateSelection = (group: CustomizationGroup, optionId: string) => {
    setSelectedCustomizations((prev) => {
      ...
      if (existing.includes(optionId)) {
        return {
          ...prev,
          [group.groupId]: existing.filter((id) => id !== optionId),
        };
      }
  ...
  const validateSelections = () => {
    for (const group of customizationGroups) {
      const selectedCount = (selectedCustomizations[group.groupId] || [])
        .length;
      const min = Number(group.min || 0);
      const max = Number(group.max || 0);
      if (min > 0 && selectedCount < min) {
        notifyOrAlert(
          `Please select at least ${min} option(s) in ${group.groupName}.`,
          "warning",
        );
        return false;
      }
      if (max > 0 && selectedCount > max) {
        notifyOrAlert(
          `Please select maximum ${max} option(s) in ${group.groupName}.`,
          "warning",
        );
        return false;
      }
                          ...
                          return (
                            <label key={option.id} className={styles.optionRow}>
                              <input
                                ...
                                checked={checked}
                                onChange={() =>
                                  updateSelection(group, option.id)
                                }
                              />
            ...
            if (useServerCart) {
              if (product.hasVariants) {
                ...
                try {
                  ...
                  notifyOrAlert(
                    getApiErrorMessage(error, "Unable to load customizations."),
                    "error",
                  );
                } finally {
  ...
  return (
    ...
    >
      ...
      <span className={styles.quantity}>
        {isUpdating ? <span className={styles.countLoader} /> : quantity}
      </span>
      <button
        type="button"
        className={styles.stepButton}
